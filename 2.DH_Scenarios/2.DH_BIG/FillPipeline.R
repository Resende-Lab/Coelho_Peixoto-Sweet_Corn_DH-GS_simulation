...
for(year in 1:7){
  ...
  #Year 1
  ####>>>>----Public program
  F1_public = randCross(Parents_public_update, nCrosses_public*nBIG, ignoreSexes = TRUE) # Spring
  ...
  if(year<7){
    ...
    F4_public.sel = selectInd(F4_public.sel, nInd = 300*nBIG, selectTop = TRUE) # Winter
  ...
  if(year<6){
    ...
    F5_public.sel = selectInd(F5_public.sel, nInd = 150*nBIG, selectTop = TRUE) # Fall
    ...
    F5_public.TC = selectInd(F5_public.sel,nInd=100*nBIG, selectTop = TRUE) # Winter
  ...
  if(year<5){
    ...
    DHTrain2@pheno = as.matrix(aggregate(as.matrix(calcGCA(PublicHybrid_TC2)$GCAf[,2])~as.matrix(calcGCA(PublicHybrid_TC2)$GCAf[,1]), FUN=mean)[,2])
  ...
  if(year<4){
    ...
    DHTrain3@pheno = as.matrix(aggregate(as.matrix(calcGCA(PublicHybrid_TC3)$GCAf[,2])~as.matrix(calcGCA(PublicHybrid_TC3)$GCAf[,1]), FUN=mean)[,2])
